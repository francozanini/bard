...
import {View, Animated, Text, ScrollView, StyleSheet, Image, Pressable, FlatList} from "react-native";
...
export default function App() {
    ...
    if (!rss || rss.items?.length === 0) return (<View><Text>No items</Text></View>);

    return (
        <>
            <ScrollView style={styles.container}>
                <Image source={{uri: rss.items[0].itunes.image}} style={styles.cardImage}/>
...
function EpisodeListItem({episode}: { episode: Episode }) {
    ...
    return <View style={styles.listItem}>
        ...
        <View style={{flex: 1, display: 'flex', flexDirection: 'row', justifyContent: "space-between"}}>
            ...
            <Text style={{color: "#6a6a6a"}}>{bitsToHourMinutesSeconds(episode.enclosures[0].length)}</Text>
        </View>
    </View>
}
...
function AudioPlayer({episode}: { episode: Episode }) {
    ...
    async function playSound() {
        ...
        const {sound} = await Audio.Sound.createAsync(
            {uri: url}, {}, status => {
                if (status.isLoaded) {
                    setAudioStatus({positionMillis: status.positionMillis, durationMillis: status.durationMillis!});
                }
            }
        );
        setSound(sound);
    ...
    return (
        <View>
            ...
            <View style={{display: 'flex', flexDirection: 'row', alignItems: 'center', padding: 16, gap: 12}}>
                ...
                <View>
                    <Text>{millisToMinutes(audioStatus.durationMillis - audioStatus.positionMillis)}</Text>
                </View>
            </View>
        </View>
    );
